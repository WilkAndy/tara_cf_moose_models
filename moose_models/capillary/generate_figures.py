...
def pc(s):
    if s <= 0.0 or s >= 1.0:
        return 0.0 # not really true for s=1, but makes no difference to result
    m = 0.5
    alpha = 1E-5
    return np.power(np.power(s, -1.0 / m) - 1, 1.0 - m) / alpha * 1E-6
...
def moose(fn):
    try:
        ...
        data = ([d[0] for d in data], [d[4] for d in data], [d[5] for d in data], [d[6] for d in data], [d[7] for d in data], [d[8] for d in data], [d[9] for d in data], [d[11] for d in data])
        f.close()
    except:
        sys.stderr.write("Cannot read " + fn + ", or it contains erroneous data\n")
        sys.exit(1)
...
moose_timesteps = ["0062", "0131", "0309", "0869"]
moosePTSUSS = [moose("cap_pp_csv_ptsuss_" + ts + ".csv") for ts in moose_timesteps]
...
# Pressure
axes[0, 0].plot(moosePTSUSS[0][0], [(p - 18.3E6)/1E6 for p in moosePTSUSS[0][7]], 'b*', markersize=4, markevery = mres, label = 'MOOSE (1 hour)')
axes[0, 0].plot(tara_pp[0][0], tara_pp[0][1], 'b-', label = 'LaForce (1 hour)')
axes[0, 0].plot(moosePTSUSS[1][0], [(p - 18.3E6)/1E6 for p in moosePTSUSS[1][7]], 'rs', markersize=4, markevery = mres, label = 'MOOSE (1 day)')
axes[0, 0].plot(tara_pp[1][0], tara_pp[1][1], 'r-', label = 'LaForce (1 day)')
axes[0, 0].plot(moosePTSUSS[2][0], [(p - 18.3E6)/1E6 for p in moosePTSUSS[2][7]], 'g^', markersize=4, markevery = mres, label = 'MOOSE (1 month)')
axes[0, 0].plot(tara_pp[2][0], tara_pp[2][1], 'g-', label = 'LaForce (1 month)')
axes[0, 0].plot(moosePTSUSS[3][0], [(p - 18.3E6)/1E6 for p in moosePTSUSS[3][7]], 'ko', markersize=4, markevery = mres, label = 'MOOSE (5 years)')
...
# Save the figure
plt.tight_layout()
plt.savefig("../../figures/laforce_comparison_cap.pdf")
